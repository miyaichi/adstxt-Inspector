...
export const SummaryPanel: React.FC<SummaryPanelProps> = ({
 ...
 if (analyzing) {
   return (
     <div className="p-4">
       <div className="bg-blue-50 text-blue-700 p-4 rounded-lg">Analyzing data...</div>
     </div>
   );
 }
 ...
 const transactionPatterns = {
   ...
   otherDirect: directEntries.filter(e => 
     e.domain !== ownerDomain && e.domain !== managerDomain
   ).length,
   otherReseller: resellerEntries.filter(e => 
     e.domain !== ownerDomain && e.domain !== managerDomain
   ).length
 };
   ...
   <div className="p-4 space-y-4">
     ...
     <div className="bg-white rounded-lg shadow overflow-hidden">
       <div className="border-b border-gray-200 p-4">
         <h3 className="text-lg font-medium text-gray-900">Summary Overview</h3>
       </div>
     ...
     <div className="bg-white rounded-lg shadow overflow-hidden">
       <div className="border-b border-gray-200 p-4">
         <h3 className="text-lg font-medium text-gray-900">Transaction Patterns</h3>
       </div>
       <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-4">
         <div className="space-y-2">
           ...
           <div className="bg-blue-50 p-2 rounded">Reseller: {transactionPatterns.ownerReseller}</div>
         </div>
         <div className="space-y-2">
           <h4 className="font-medium">Manager Domain Transactions</h4>
           <div className="bg-green-50 p-2 rounded">Direct: {transactionPatterns.managerDirect}</div>
           <div className="bg-green-50 p-2 rounded">Reseller: {transactionPatterns.managerReseller}</div>
         </div>
         <div className="col-span-2 space-y-2">
           <h4 className="font-medium">Other Domain Transactions</h4>
           <div className="bg-yellow-50 p-2 rounded">Direct: {transactionPatterns.otherDirect}</div>
           <div className="bg-yellow-50 p-2 rounded">Reseller: {transactionPatterns.otherReseller}</div>
         </div>
       </div>
     </div>
     ...
     <div className="bg-white rounded-lg shadow overflow-hidden">
       <div className="border-b border-gray-200 p-4">
         <h3 className="text-lg font-medium text-gray-900">Seller Type Analysis</h3>
       </div>
       <div className="p-4 space-y-4">
         {sellerAnalysis.map(analysis => {
           const sellers = analysis.sellersJson?.data || [];
           const publisherCount = sellers.filter(s => s.seller_type?.toUpperCase() === 'PUBLISHER').length;
           const intermediaryCount = sellers.filter(s => s.seller_type?.toUpperCase() === 'INTERMEDIARY').length;
           ...
           return (
             <div key={analysis.domain} className="bg-gray-50 p-4 rounded-lg">
               ...
               <div className="grid grid-cols-3 gap-2 mt-2">
                 ...
                 <div className="bg-yellow-50 p-2 rounded">Intermediaries: {intermediaryCount}</div>
